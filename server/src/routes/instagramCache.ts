...
function syncAuthorized(secret: string | undefined, header: string | undefined): boolean {
...
export const instagramCacheRoutes: FastifyPluginAsync = async app => {
  ...
  app.post(
    '/api/instagram-cache/sync',
    {
      schema: {
        ...
        description:
          ...
          'Se INSTAGRAM_SYNC_SECRET estiver definido, envie o mesmo valor no header X-Instagram-Sync-Secret.',
        response: {
    ...
    async (request, reply) => {
      ...
      const header = request.headers['x-instagram-sync-secret']
      const headerVal = typeof header === 'string' ? header : undefined

      if (!syncAuthorized(secret, headerVal)) {
